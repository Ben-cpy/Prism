...
// ggml_compute_forward_max_rows

static void ggml_compute_forward_max_rows_f32(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    ...
    for (int64_t i3 = 0; i3 < ne03; i3++) {
        for (int64_t i2 = 0; i2 < ne02; i2++) {
            for (int64_t i1 = 0; i1 < ne01; i1++) {
                ...
                ggml_vec_max_f32(ne00, &row_max, src_row);
                dst_row[0] = row_max;
            }
        }
    }
...
void ggml_compute_forward_max_rows(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_max_rows_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
