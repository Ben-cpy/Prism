...
bool llm_graph_input_attn_kv::can_reuse(const llm_graph_params & params) {
  ...
  //res &= self_v_idxs->ne[0] == params.ubatch.n_tokens; // TODO: need to move this to the unified cache and check there
    ...
    const bool h2o_enabled = cparams.h2o_local_window + cparams.h2o_heavy_budget > 0;
    if (h2o_enabled && params.current_phase == 2) {
        const bool use_inter_new = params.ubatch.pos && params.ubatch.pos[0] > 0 && mctx->h2o_is_memory_initialized();
        if (use_inter_new != h2o_use_inter) {
            return false;
        }
    }
...
bool llm_graph_input_h2o_intra::can_reuse(const llm_graph_params & params) {
    ...
    const llama_pos pos0 = params.ubatch.pos ? params.ubatch.pos[0] : -1;
    if (pos0 != ubatch_pos0) {
        return false;
    }
...
ggml_tensor * llm_graph_context::build_attn_mha_h2o(
         ...
         ggml_tensor ** attn_logits_out) const {
    const bool v_trans = v->nb[1] > v->nb[2];
    const auto n_stream = k->ne[3];
    ...
    if (attn_logits_out != nullptr) {
        ggml_tensor * kq_scaled = ggml_scale(ctx0, kq, kq_scale);
        if (kq_mask) {
            ggml_tensor * kq_mask_cnv = kq_mask;
            if (kq_mask_cnv->type != kq_scaled->type) {
                kq_mask_cnv = ggml_cast(ctx0, kq_mask_cnv, kq_scaled->type);
            }
            kq_scaled = ggml_add(ctx0, kq_scaled, kq_mask_cnv);
        }
        *attn_logits_out = kq_scaled;
    }
...
ggml_tensor * llm_graph_context::build_attn_h2o(
    ...
    ggml_tensor * cur = nullptr;
    ggml_tensor * intra_weights = nullptr;
    ggml_tensor * intra_logits = nullptr;

    bool did_fuse = false;
    if (current_phase == 2) {
        // Ensure phase-2 graph allocates H2O input buffers even when they are not directly used.
        ggml_build_forward_expand(gf, inp->get_k_idxs());
        ggml_build_forward_expand(gf, inp->get_v_idxs());
        ggml_build_forward_expand(gf, inp->get_kq_mask());
        ...
        // Ensure phase-2 intra cache inputs are allocated even when only viewed.
        ggml_build_forward_expand(gf, intra_o_full);
        ggml_build_forward_expand(gf, intra_l_full);
        ggml_build_forward_expand(gf, intra_m_full);
        ...
        if (std::getenv("H2O_DEBUG_USE_INTER") && il == 0) {
            const long long pos0 = ubatch.pos ? (long long) ubatch.pos[0] : -1;
            fprintf(stderr, "[H2O] phase=%d ubatch.pos[0]=%lld use_inter=%d\n",
                    current_phase, pos0, use_inter ? 1 : 0);
        }
        ...
        if (use_inter) {
            ...
            ggml_tensor * k_mem = mctx_kv->h2o_gather_k_memory(ctx0, il);
            ggml_tensor * v_mem = mctx_kv->h2o_gather_v_memory(ctx0, il);

            ggml_tensor * inter_out = build_attn_inter_h2o(
                q, k_mem, v_mem, kq_scale, il, &inter_logits, &inter_weights);

            if (inter_weights != nullptr && res != nullptr) {
                ggml_tensor * inter_colsum = build_attn_colsum(inter_weights, il);
            ...
            ggml_tensor * intra_l4 = ggml_view_4d(ctx0, intra_l,
                    1, intra_l->ne[0], intra_l->ne[1], intra_l->ne[2],
                    intra_l->nb[0], intra_l->nb[1], intra_l->nb[2],
                    0);
            if (state.l && intra_l4->type != state.l->type) {
                intra_l4 = ggml_cast(ctx0, intra_l4, state.l->type);
            }

            ggml_tensor * intra_m4 = ggml_view_4d(ctx0, intra_m,
                    1, intra_m->ne[0], intra_m->ne[1], intra_m->ne[2],
                    intra_m->nb[0], intra_m->nb[1], intra_m->nb[2],
                    0);
    ...
    if (attn_weights_out != nullptr) {
        *attn_weights_out = intra_weights;
    }
...
namespace {
    ...
    ggml_tensor * h2o_as_per_head_output(ggml_context * ctx, ggml_tensor * out, ggml_tensor * logits) {
        ...
        const int64_t n_head   = logits->ne[2];
        const int64_t n_stream = logits->ne[3];

        if (out->ne[1] == n_tokens && out->ne[2] == n_head && out->ne[3] == n_stream) {
            return out;
        }

        if (out->ne[2] == 1 && out->ne[3] == 1) {
            const int64_t n_embd = out->ne[0];
            GGML_ASSERT(n_embd % n_head == 0);
            GGML_ASSERT(out->ne[1] == n_tokens * n_stream);

            const int64_t n_embd_head = n_embd / n_head;
            ggml_tensor * out4 = ggml_reshape_4d(ctx, out, n_embd_head, n_head, n_tokens, n_stream);
            return ggml_permute(ctx, out4, 0, 2, 1, 3);
        }
...
ggml_tensor * llm_graph_context::build_attn_inter_h2o(
        ggml_tensor * q,
        ggml_tensor * k_mem,
        ggml_tensor * v_mem,
              float   kq_scale,
    ...
    const bool v_trans = v_mem->nb[1] > v_mem->nb[2];
    const auto n_stream = k_mem->ne[3];

    q = ggml_view_4d(ctx0, q, q->ne[0], q->ne[1], q->ne[2]/n_stream, n_stream,
                     q->nb[1], q->nb[2], q->nb[3]/n_stream, 0);

    q = ggml_permute(ctx0, q, 0, 2, 1, 3);
    ggml_tensor * k = ggml_permute(ctx0, k_mem, 0, 2, 1, 3);
    ggml_tensor * v = ggml_permute(ctx0, v_mem, 0, 2, 1, 3);

    if (std::getenv("LLAMA_H2O_DEBUG_SHAPES")) {
        fprintf(stderr,
                "[H2O] q ne=[%lld,%lld,%lld,%lld] k_mem ne=[%lld,%lld,%lld,%lld] k ne=[%lld,%lld,%lld,%lld]\n",
                (long long) q->ne[0], (long long) q->ne[1], (long long) q->ne[2], (long long) q->ne[3],
                (long long) k_mem->ne[0], (long long) k_mem->ne[1], (long long) k_mem->ne[2], (long long) k_mem->ne[3],
                (long long) k->ne[0], (long long) k->ne[1], (long long) k->ne[2], (long long) k->ne[3]);
    }

    ggml_tensor * kq = ggml_mul_mat(ctx0, k, q);
    cb(kq, "kq_inter_h2o", il);

    ggml_mul_mat_set_prec(kq, GGML_PREC_F32);

    if (arch == LLM_ARCH_GROK) {
        kq = ggml_tanh(ctx0, ggml_scale(ctx0, kq, hparams.f_attn_out_scale / hparams.f_attn_logit_softcapping));
        cb(kq, "kq_tanh_inter_h2o", il);
        kq = ggml_scale(ctx0, kq, hparams.f_attn_logit_softcapping);
        cb(kq, "kq_scaled_inter_h2o", il);
    }

    if (hparams.attn_soft_cap) {
        kq = ggml_scale(ctx0, kq, 1.0f / hparams.f_attn_logit_softcapping);
        cb(kq, "kq_scaled_1_inter_h2o", il);
        kq = ggml_tanh(ctx0, kq);
        cb(kq, "kq_tanh_inter_h2o", il);
        kq = ggml_scale(ctx0, kq, hparams.f_attn_logit_softcapping);
        cb(kq, "kq_scaled_2_inter_h2o", il);
    }

    if (inter_logits_out != nullptr) {
        *inter_logits_out = ggml_scale(ctx0, kq, kq_scale);
    }

    kq = ggml_soft_max_ext(ctx0, kq, nullptr, kq_scale, 0.0f);
    cb(kq, "kq_soft_max_inter_h2o", il);

    if (inter_weights_out != nullptr) {
        *inter_weights_out = kq;
    }

    if (!v_trans) {
        v = ggml_cont(ctx0, ggml_transpose(ctx0, v));
        cb(v, "v_cont_inter_h2o", il);
    }
    if (ggml_is_transposed(v)) {
        v = ggml_cont(ctx0, v);
        cb(v, "v_cont_inter_h2o", il);
    }
    ...
    if (std::getenv("LLAMA_H2O_DEBUG_SHAPES")) {
        fprintf(stderr,
                "[H2O] v ne=[%lld,%lld,%lld,%lld] kq ne=[%lld,%lld,%lld,%lld] v_trans=%d\n",
...
static std::unique_ptr<llm_graph_input_attn_kv> build_attn_inp_kv_impl(
    ...
    const bool h2o_enabled = cparams.h2o_local_window + cparams.h2o_heavy_budget > 0;
    if (h2o_enabled && ubatch.pos) {
        inp->h2o_use_inter = ubatch.pos[0] > 0 && mctx_cur->h2o_is_memory_initialized();
    }
...
llm_graph_input_h2o_intra * llm_graph_context::build_h2o_intra_inp() const {
    ...
    inp->ubatch_pos0 = ubatch.pos ? ubatch.pos[0] : -1;
